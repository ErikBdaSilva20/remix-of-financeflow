...
const Reports = () => {
  ...
  const { data: expenseCategories, isLoading: expenseLoading } = useExpenseCategories(filters.dateRange);
  ...
  const fmt = (amount: number, date?: string) => {
    const v = convertAmount(amount, 'USD', date);
    return `${currencySymbol}${v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  };

  const revenue = metrics?.find(m => m.metric_type === 'revenue');
  const mrr = metrics?.find(m => m.metric_type === 'mrr');
  const totalExpenses = expenseCategories?.reduce((s, e) => s + e.amount, 0) ?? 0;
                ...
                <table className="w-full text-sm">
                  <tbody>
                    ...
                    <tr className="border-t border-border">
                      ...
                      <td className="px-6 py-3 text-right font-medium">{fmt(mrr?.amount ?? 0)}</td>
                      ...
                      <tbody>
                        {revenueSources.map((src, i) => (
                          <tr key={i} className="border-b border-border last:border-0">
                            ...
                            <td className="px-6 py-3 text-right font-medium">{fmt(src.amount)}</td>
                      ...
                      <tbody>
                        {expenseCategories.map((exp, i) => (
                          <tr key={i} className="border-b border-border last:border-0">
                            ...
                            <td className="px-6 py-3 text-right font-medium">{fmt(exp.amount)}</td>
                            <td className="px-6 py-3 text-right text-muted-foreground">
                      ...
                      <tbody>
                        {topClients.slice(0, 8).map((client, i) => (
                          <tr key={i} className="border-b border-border last:border-0">
                            ...
                            <td className="px-6 py-3 text-right">{fmt(client.revenue)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                      ...
                      <tbody>
                        {vendors.slice(0, 8).map((vendor, i) => (
                          <tr key={i} className="border-b border-border last:border-0">
                            ...
                            <td className="px-6 py-3 text-right">{fmt(vendor.amount)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
